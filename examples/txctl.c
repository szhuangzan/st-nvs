/***----------------------------------------------------------------***
 *** Rialto Transaction Co-ordinator
 ***----------------------------------------------------------------***/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
#define MAX_WAIT_THREADS_DEFAULT 5
...
int main(int argc, char *argv[])
{
  ...
  /* Start the daemon */
  if (!interactive_mode) start_daemon();
...
static void usage(const char *progname)
{
  fprintf(stderr, "Usage: %s -d <root_directory> [<options>]\n\n"
          "Possible options:\n\n"
...
static void parse_arguments(int argc, char *argv[])
{
  ...
  while ((opt = getopt(argc, argv, "d:ih")) != EOF) {
    switch (opt) {
    case 'd':
      rootdir = optarg;
      break;
...
void load_config(void)
{
 char path[500];
  strcpy(path,rootdir);
  strcat(path,"/");
  strcat(path,"txctl.conf");
/* do something for a config struct here */
  if(conf != NULL)
  err_report(errfd, "INFO: process %d (pid %d): configuration loaded",
             my_index, my_pid);
  else
  err_report(errfd, "ERR: process %d (pid %d): %s not found",
             my_index, my_pid,path);
}
